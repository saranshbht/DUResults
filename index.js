let sorter = (a, b) => {
  ...
  if (isNaN(float_a) && isNaN(float_b)) return a <= b ? -1 : 1;
  if (isNaN(float_a) || isNaN(float_b)) return a.length <= b.length ? -1 : 1;
...
let createTable = (keys, objects) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (i == 1 || i == 2) columns[i].type = "string";
    else columns[i].type = "num";
  }
...
//https://raw.githubusercontent.com/saranshbht/DUResults/development/Marks/Semester/16/570/015.csv
...
let base_url =
  "https://raw.githubusercontent.com/saranshbht/DUResults/development/";
